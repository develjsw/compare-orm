...
export abstract class TransactionService {
    ...
    protected async startTransaction(): Promise<void> {
        if (!this.preparedQueryRunner) await this.queryRunner.startTransaction();
    }

    protected async commitTransaction(): Promise<void> {
        if (!this.preparedQueryRunner) await this.queryRunner.commitTransaction();
    }

    protected async rollbackTransaction(): Promise<void> {
        if (!this.preparedQueryRunner) await this.queryRunner.rollbackTransaction();
    }
}
